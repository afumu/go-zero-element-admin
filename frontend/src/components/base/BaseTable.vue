  ...
  methods: {
    ...
    deleteById(row, childConfirm = true) {
      ...
      this.$dialog
        ...
        .then(() => {
          ...
          this.api
            .deleteById(row[this.configData['field.id']])
            .then(() => {
    ...
    deleteByIdInBatch(childConfirm = true) {
      ...
      this.$dialog
        ...
        .then(() => {
          ...
          this.api
            .deleteByIdInBatch(
              this.tableData.selectedRows.map((row) => row.id).join(',')
            )
            .then(() => {
              this.$tip.apiSuccess('删除成功')
    ...
    __afterDelete(deleteCount = 1) {
      ...
      if (this.tableData.list.length - deleteCount === 0) {
        this.handlePageChange(
          this.tableData.pagination.pageIndex - 1 === 0
            ? 1
    ...
    __checkApi() {
      if (this.api == null) {
        throw new Error(
          "The page is not initialized, you can use method 'this.config' to initialize this page."
        )
      }
    },
  },
