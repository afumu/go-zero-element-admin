<template>
  <GlobalWindow :visible.sync="visible" :confirm-working="isWorking" width="582px" title="配置角色权限" @confirm="confirm">
    <p class="tip" v-if="role != null">为角色 <em>{{role.name}}</em> 配置权限</p>
    <p class="tip-warn"><i class="el-icon-warning"></i>提醒：权限配置后需重新登录后生效</p>
    <el-transfer ref="permissionTransfer" v-model="selectedIds" filterable :filter-method="filterPermissions" :titles="['未授权权限', '已授权权限']" :props="{
        key: 'id',
        label: 'name'
      }" :data="permissions">
    </el-transfer>
...
export default {
  ...
  methods: {
    ...
    filterPermissions(query, item) {
      ...
      return (
        item.code.toLowerCase().indexOf(lowerCaseQuery) > -1 ||
